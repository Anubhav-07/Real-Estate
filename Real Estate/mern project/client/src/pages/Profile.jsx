...
export default function Profile() {
  ...
  const [formDatata, setFormDatata] = useState({});
    ...
    uploadTask.on('state_changed',
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          console.log('File available at', downloadURL);  // Check the URL here
          setFormDatata({ ...formDatata, avatar: downloadURL });
        });
      }
    );
  };
  ...
  const handleChange = (e) => {
    setFormDatata({ ...formDatata, [e.target.id]: e.target.value });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`/api/user/update/${currentUser._id}`, {
        ...
        body: JSON.stringify(formDatata),
      });
  ...
  const handleShowListings = async () => {
    try {
      ...
      console.error('Error in handleShowListings:', error);
      setShowListingsError(true);
    }
  };
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <img onClick={() => fileRef.current.click()} src={formDatata.avatar || currentUser.avatar} alt="profile" className='rounded-full h-24 w-24 object-cover cursor-pointer self-center mt-2' />
